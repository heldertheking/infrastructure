...
```
panel → pelican-net → database (MariaDB :3306)
      → pelican-net → redis (:6379)
      → public-net  → exposed via Traefik (pelican.<domain>)

wings → pelican-net → communicates with panel
      → public-net  → exposed via Traefik (wings.<domain>)
      → /var/run/docker.sock → manages game server containers
```
